...
import { packagingHeaders } from "./data/packaging-fields";
import PackagingAddOrEdit from "./modal/PackagingAddOrEdit.vue";

const headers = ref(packagingHeaders);
const pagination = ref<PaginateModel<PackagingModel>>({
  data: [],
  current_page: 1,
  from: 0,
  last_page: 0,
...
const modal = ref<ModalProps<PackagingModel>>({
  show: false,
...
const openModal = (actions: "EDIT" | "DELETE" | "ADD", item?: PackagingModel) => {
  switch (actions) {
    case "EDIT":
      modal.value.show = true;
      modal.value.title = "Editar Presentacion";
    ...
    case "DELETE":
      deleteConfirm({
        ...
        icon: "warning",
        fnAction: async () => {
          const result = await PackagingService.delete(item?.id ?? 0);
          return result;
        },
        fnReload: () => {
    ...
    case "ADD":
      modal.value.show = true;
      modal.value.title = "Registrar Presentacion";
...
const list = async (opt: ITableOptions) => {
  try {
    loading.value = true;
    const result = await PackagingService.list(opt);
