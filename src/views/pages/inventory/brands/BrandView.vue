...
import { brandsFields } from "./data/brand.field";
import BrandAddOrEdit from "./modal/BrandAddOrEdit.vue";

const headers = ref(brandsFields);
const pagination = ref<PaginateModel<BrandModel>>({
  data: [],
  current_page: 1,
  from: 0,
  last_page: 0,
...
const modal = ref<ModalProps<BrandModel>>({
  show: false,
...
const openModal = (actions: "EDIT" | "DELETE" | "ADD", item?: BrandModel) => {
  switch (actions) {
    case "EDIT":
      modal.value.show = true;
      modal.value.title = "Editar Marca";
    ...
    case "DELETE":
      deleteConfirm({
        ...
        icon: "warning",
        fnAction: async () => {
          const result = await BrandService.delete(item?.id ?? 0);
          return result;
        },
        fnReload: () => {
    ...
    case "ADD":
      modal.value.show = true;
      modal.value.title = "Registrar Marca";
...
const list = async (opt: ITableOptions) => {
  try {
    loading.value = true;
    const result = await BrandService.list(opt);
...
<template>
  <Filter title="Marcas" v-model:search="search">
    <template #others>
